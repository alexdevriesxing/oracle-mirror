...
function cards(items: readonly Entry[], base: string, kind: string) {
  return `<div class="divination-grid">${items.map((item) => `<a class="divination-card" href="${base}/${item.slug}"><span class="divination-glyph" aria-hidden="true">${kind === "crystal" ? "◇" : kind === "tea" ? "☕" : kind === "wax" ? "🕯" : kind === "scry" ? "◉" : "⌁"}</span><h2>${esc(item.name)}</h2><p>${esc(item.theme)}</p></a>`).join("")}</div>`;
}
...
export function handleDivinationRoute(path: string): Response {
  ...
  else if (normalized === "/divination/guides") html = collection("Grounded Divination Guides", "Learn pattern recognition, symbol journaling, ritual safety and the distinction between tradition and evidence.", normalized, "These guides explain how to preserve the imaginative value of divination while keeping claims proportional to the evidence.", DIVINATION_GUIDES, "A useful reading distinguishes observation, inherited symbolism, personal association and factual evidence instead of blending them into one claim.", "guide");
